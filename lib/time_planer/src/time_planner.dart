import 'package:edtech/time_planer/src/time_planner_style.dart';
import 'package:edtech/time_planer/src/time_planner_task.dart';
import 'package:edtech/time_planer/src/time_planner_time.dart';
import 'package:edtech/time_planer/src/time_planner_title.dart';
...
import '../../features/weekly_calendar/controllers/weekly_calendar_controller.dart';
...
class _TimePlannerState extends State<TimePlanner> {
  ...
  void initState() {
    _initData();
    super.initState();
    controller.scrollToCurrentHour(config.cellHeight!, 7);

    // Future.delayed(Duration.zero).then((_) {
    //   int hour = DateTime.now().hour;
    //   double scrollOffset = (hour - widget.startHour) * config.cellHeight!.toDouble();
    //   controller.timeVerticalController.animateTo(
    //     scrollOffset,
  ...
  @override
  void dispose() {
    controller.timeVerticalController.dispose();
    super.dispose();
  }
      ...
      children: <Widget>[
        ...
        Expanded(
          child: SingleChildScrollView(
            controller: controller.timeVerticalController,
            child: Stack(
              children: [
                Row(
                  mainAxisSize: MainAxisSize.min,
...
class DashedDivider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final boxSize = isVertical
            ? constraints.constrainHeight()
            : constraints.constrainWidth();
        final dashCount = (boxSize / (dashWidth + dashSpace)).floor();
        return Flex(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          direction: isVertical ? Axis.vertical : Axis.horizontal,
          children: List.generate(dashCount, (_) {
            return SizedBox(
              width: isVertical ? dashHeight : dashWidth,
              height: isVertical ? dashWidth : dashHeight,
              child: DecoratedBox(
                decoration: BoxDecoration(color: color),
              ),
            );
